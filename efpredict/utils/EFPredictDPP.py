        ...
        with open(os.path.join(self.output, "log.csv"), "a") as f:
            ...
            for epoch in range(epoch_resume, self.num_epochs):
                ...
                if torch.cuda.current_device() == 0:
                    self._save_checkpoint(self, epoch, self.output, loss, bestLoss, y, yhat, optim, scheduler)
...
def run(
    ...
    # https://www.youtube.com/watch?v=-K3bZYHYHEA&list=PL_lsbAsL_o2CSuhUhJIiW0IkdT5C2wGWj
    # TODO Implement DistributedDataParallel
    if device.type == "cuda":
        current_device = torch.cuda.current_device()
        all_devices = list(range(torch.cuda.device_count()))
        model = DDP(model, device_ids=[all_devices], output_device=current_device)
    else:
        model.to(device)
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        for epoch in range(epoch_resume, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                loss, yhat, y = efpredict.utils.EFPredictDPP.run_epoch(model, dataloader, phase == "train", optim, device)
                f.write("{},{},{},{},{},{},{},{},{}\n".format(epoch,
                                                              phase,
                                                              loss,
                                                              sklearn.metrics.r2_score(y, yhat),
